...
domain_check() {
    echo -e " ${RedBG} 安装完成，CTRL+C退出安装界面${Font}"
    read -rp "别忘记手动在甲骨文云设置入站规则" domain
    ...
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        ...
        echo -e " ${RedBG} 安装完成，CTRL+C退出安装界面${Font}"
        echo -e " ${RedBG} 安装完成，CTRL+C退出安装界面${Font}" && read -r install
        case $install in
        [yY][eE][sS] | [yY])
            echo -e "${GreenBG} 继续安装 ${Font}"
            sleep 2
...
menu() {
    echo -e "—————————————————————————————————————————————————————————————————"""
    echo -e "${Green}1.${Font}安装甲骨文云VPS环境,centos8正常使用，其它系统未测试。"
    echo -e "—————————————————————————————————————————————————————————————————"""
